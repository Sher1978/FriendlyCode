import 'package:flutter_test/flutter_test.dart';
import 'package:friendly_code/core/logic/reward_calculator.dart';

void main() {
  group('RewardCalculator Logic v1.2 (Premium Rules)', () {
    final now = DateTime(2026, 1, 1, 12, 0, 0); // Reference time

    // Setup the specific rules requested by Owner
    setUp(() {
      RewardCalculator.updateConfig(const RewardConfig(
        tier1Hours: 24, // < 24h
        tier2Hours: 36, // < 36h
        tier3Hours: 240, // < 10 days
        rewardTier1: 20,
        rewardTier2: 15,
        rewardTier3: 10,
        rewardBase: 5,
      ));
    });

    test('Visit within 24h gives Max Reward (20%)', () {
      final lastVisit = now.subtract(const Duration(hours: 23));
      expect(RewardCalculator.calculate(lastVisit, now), 20);
    });

    test('Visit between 24h and 36h gives Mid Reward (15%)', () {
      // 30 hours ago
      final lastVisit = now.subtract(const Duration(hours: 30));
      expect(RewardCalculator.calculate(lastVisit, now), 15);
    });

    test('Visit exactly at 36h gives Mid Reward (15%)', () {
      final lastVisit = now.subtract(const Duration(hours: 36));
      expect(RewardCalculator.calculate(lastVisit, now), 15);
    });

    test('Visit between 36h and 10 days gives Low Reward (10%)', () {
      // 5 days ago (120 hours)
      final lastVisit = now.subtract(const Duration(days: 5));
      expect(RewardCalculator.calculate(lastVisit, now), 10);
    });

    test('Visit after 10 days gives Retention Base (5%)', () {
      // 11 days ago
      final lastVisit = now.subtract(const Duration(days: 11));
      expect(RewardCalculator.calculate(lastVisit, now), 5);
    });

    test('Admin Configuration: Can change rules dynamically', () {
      // Admin changes rules: 24h = 50%
      RewardCalculator.updateConfig(const RewardConfig(
        rewardTier1: 50,
      ));
      
      final lastVisit = now.subtract(const Duration(hours: 10));
      expect(RewardCalculator.calculate(lastVisit, now), 50);
    });
  });
}

...
def patch_new_qr_page():
    path = r"c:\Sher_AI_Studio\projects\FriendlyCode\src\NewQRPage.jsx"
    if not os.path.exists(path): return
    with open(path, "r", encoding="utf-8") as f:
        content = f.read()
    
    # Fix the syntax error literal \n
    content = content.replace("useState(false);\\n    const [hasInteracted", "useState(false);\n    const [hasInteracted")
    
    with open(path, "w", encoding="utf-8") as f:
        f.write(content)
    print("NewQRPage syntax fixed")
...
def patch_unified_activation():
    ...
    # Define color logic helper inside the script to replace the component
    NEW_COMPONENT = """
...
const VipGiftTeaser = ({ tiers, ambientColor, discountValue }) => {
    ...
    const rows = React.useMemo(() => {
        ...
        return tiers
            ...
            .sort((a, b) => b.percentage - a.percentage) // Sort by discount high to low
            .map(t => {
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        >
                            <span style={{
                                fontSize: "32px",
                                fontWeight: 900,
                                color: activeColor, // Tier-specific color for the number
                                letterSpacing: "-1px",
                                textShadow: `0 0 25px ${activeColor}40`,
                            }}>{current.pct}%</span>
                            <span style={{
                                fontSize: "12px", fontWeight: 800,
                                color: activeColor, // Tier-specific color for the duration
                                opacity: 0.8,
...
"""
    ...
    # Replace the VipGiftTeaser component block
    import re
    # Match from // VIP Gift Teaser Component until }; before const UnifiedActivation
    ...
    if re.search(pattern, content, re.DOTALL):
        ...
        print("UnifiedActivation VipGiftTeaser replaced")
    else:
        print("Could not find VipGiftTeaser component in UnifiedActivation")

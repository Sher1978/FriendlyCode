...
import { faStar, faChevronRight, faChevronDown, faClock, faUser, faExclamationTriangle, faGift, faWallet, faChartPie, faTimes, faArrowUp, faArrowDown, faWifi, faReceipt, faBolt } from '@fortawesome/free-solid-svg-icons';
...
const NewQRPage = () => {
        ...
        const unsubscribe = onAuthStateChanged(auth, async (user) => {
            ...
            const checkUserAndVenue = async () => {
                try {
                    ...
                    const cachedEmail = userData?.email || safeStorage.getItem('guestEmail') || user?.email || '';
                    if (cachedEmail) {
                        try {
                            const mergeRes = await findAndMergeUserByEmail(db, cachedEmail, user.uid);
                            if (mergeRes.uid) {
                                targetUid = mergeRes.uid;
                                safeStorage.setItem('effectiveUid', targetUid);
                                if (targetUid !== user.uid) {
                                    const targetUserSnap = await getDoc(doc(db, 'users', targetUid));
                                    if (targetUserSnap.exists()) userData = targetUserSnap.data();
                                }
                            }
                        } catch (e) {
                            console.warn("User deduplication check error:", e);
                        }
                    }
                    ...
                    unsubscribeUser = onSnapshot(doc(db, 'users', targetUid), (docSnap) => {
                        ...
                        try {
                            ...
                            for (const currentUid of uidsToTry) {
                                ...
                                const qUidVisits = query(collection(db, 'visits'), where('uid', '==', currentUid), where('venueId', '==', activeVenueId));
                            ...
                            if (visitDocs.length === 0 && email) {
                                const qEmailVisits = query(collection(db, 'visits'), where('guestEmail', '==', email), where('venueId', '==', activeVenueId));
                        ...
                        // Override discount if Google Maps promotion is active
                        if (searchParams.get('utm_source') === 'google_maps') {
                            const lastClaimedStr = userData?.googleMapsDiscountAt || userProfile?.googleMapsDiscountAt;
                            const lastClaimed = lastClaimedStr ? new Date(lastClaimedStr).getTime() : 0;
                            const nowMs = Date.now();
                            const timeDiff = nowMs - lastClaimed;
                            ...
                            if (canClaimGoogleBonus) {
                                ...
                                result = result || {};
                                result.status = 'google_maps_bonus';
                            }
                        }
                    ...
                    const isMaxDiscount = calculatedDiscount >= 20 || result?.status === 'google_maps_bonus';
                    setPredictionState({ percent: calculatedDiscount, secondsLeft, label: result?.status || 'new', isBase: calculatedDiscount <= 5, isMax: isMaxDiscount });
                    setLiveSecondsLeft(secondsLeft);
    ...
    const isUserLoggedIn = Boolean(auth.currentUser && !auth.currentUser.isAnonymous && (auth.currentUser.email || userProfile?.email || safeStorage.getItem('guestEmail')));
    ...
    const isDepositActive = (isUserLoggedIn && depositBalance > 0) && (!hasGoogleReviewConfigured || hasCompletedReview);
    ...
    // ── DEPOSIT COLOR-CODING SYSTEM (Green -> Yellow -> Orange -> Red) ──
    ...
    if (isDepositActive) {
        ...
        if (ratio >= 0.75) {
            depositCapacity = 100; // Emerald Green (#00FF41)
        } else if (ratio >= 0.40) {
            depositCapacity = 50;  // Gold / Yellow (#FFD700)
        } else if (ratio >= 0.20) {
            depositCapacity = 25;  // Amber / Orange (#FF8C00)
        } else {
            depositCapacity = 10;  // Critical Ruby Red (#FF3131)
        }
    }
    ...
    const offerSecondsLeft = isReviewDiscountActive 
        ? Math.max(0, Math.floor(reviewMsLeft / 1000)) 
        : liveSecondsLeft;
    ...
    const getDiscountTypeInfo = () => {
        if (isDepositActive) {
            return {
                type: 'deposit',
                badge: t('badge_deposit', '💰 Deposit'),
                title: t('title_deposit', 'Deposit Discount'),
                subtitle: t('subtitle_deposit', { tier: currentDiscountTier, balance: depositBalance.toLocaleString(), currency: venueCurrency, defaultValue: `Tier ${currentDiscountTier} Active (Balance: ${depositBalance.toLocaleString()} ${venueCurrency})` }),
                color: '#D4AF37',
        ...
        if (isReviewDiscountActive && reviewCompDate) {
            let remainingStr = '';
            if (reviewMsLeft > 0) {
                remainingStr = t('expires_in_days_hours', { days: reviewDaysLeft, hours: reviewHoursLeft, defaultValue: `Expires in: ${reviewDaysLeft}d ${reviewHoursLeft}h` });
            } else {
                remainingStr = t('expires_today', 'Expires today');
            }

            return {
                type: 'google_review',
                badge: t('badge_google_review', '⭐ Google Maps Review'),
                title: t('title_google_review', 'Google Maps Review Reward'),
    ...
    const timelineItems = (() => {
        ...
        if (Array.isArray(loyaltyConfig)) {
            ...
            const colors = ['#00FF41', '#FFD700', '#FFAA00', '#FFD700'];
            ...
            return sorted.map((tier, index) => {
                ...
                return {
                    ...
                    color: perc >= 10 ? (colors[index] || '#FFD700') : '#FFAA00',
                    perc
                };
            });
        }
    ...
    return (
        ...
        >
            ...
            {/* ── TOP HEADER SECTION (Perfect Centering + Profile Circle Icon Only) ── */}
            <div 
            ...
            >
                {/* Left: User Profile Icon Only (Circle with Human Icon, No Text) */}
                ...
                {/* Center: Venue Name ALWAYS Perfectly Centered on Screen */}
                <div 
                    className="absolute left-1/2 -translate-x-1/2 max-w-[calc(100%-110px)] flex flex-col items-center text-center cursor-pointer select-none active:opacity-75 transition-opacity z-10"
                ...
                >
                    <h1 className="text-xl sm:text-2xl font-black tracking-tight text-white leading-tight truncate max-w-full drop-shadow-[0_2px_10px_rgba(0,0,0,0.8)]">
                        {(activeVenueId === 'default_venue' || !venueName) ? "Unknown" : venueName}
                    </h1>
                </div>
                ...
                <div className="w-10 h-10 flex-shrink-0 flex items-center justify-end z-20">
                    <button
                        ...
                        className="h-10 min-w-[40px] px-2.5 bg-black/50 hover:bg-black/70 backdrop-blur-xl rounded-full text-xs font-bold text-white border border-white/20 active:scale-95 transition-all uppercase shadow-lg flex items-center justify-center"
                    >
            ...
            <div className="flex flex-col z-10 pt-1">
                ...
                <div className="flex flex-col items-center px-4 w-full max-w-md mx-auto gap-2.5 py-1">
                    ...
                    {isDepositActive ? (
                        /* ── DEPOSIT HOLDER VIEW (No battery, personal QR code for deduction, color thresholded) ── */
                        (() => {
                            const depTheme = getDepositColorTheme(depositBalance, venueData);
                            return (
                                <div className={`flex flex-col items-center w-full bg-[#1C1C1E]/80 backdrop-blur-[40px] border ${depTheme.borderClass} rounded-[28px] p-5 shadow-2xl relative overflow-hidden flex-shrink-0 text-center transition-all duration-300`}>
                                    <div className="absolute top-0 right-0 w-32 h-32 rounded-full blur-2xl pointer-events-none opacity-20" style={{ backgroundColor: depTheme.color }} />

                                    <div className="flex items-center gap-2 mb-1">
                                        <FontAwesomeIcon icon={faWallet} style={{ color: depTheme.color }} className="text-sm" />
                                        <span className={`text-xs font-extrabold uppercase tracking-widest px-2.5 py-0.5 rounded-full border ${depTheme.badgeBg}`}>
                                            {t('deposit_balance_title', 'Your Deposit Balance')} ({Math.round(depTheme.pct)}%)
                                        </span>
                                    </div>
                                    ...
                                    {depTheme.isLow && (
                                        <motion.div
                                            ...
                                            animate={{ opacity: 1, y: 0 }}
                                            className="mt-3.5 p-3 w-full bg-gradient-to-r from-red-950/90 via-red-900/80 to-red-950/90 border-2 border-red-500 rounded-2xl text-center shadow-[0_0_25px_rgba(255,49,49,0.5)]"
                                        >
                        ...
                        <div className="flex flex-col items-center w-full bg-[#1C1C1E]/60 backdrop-blur-[40px] border border-white/10 rounded-[28px] p-4 shadow-2xl relative overflow-hidden flex-shrink-0">
                            ...
                            {/* ── DISCOUNT TYPE INFO DIRECTLY INSIDE BATTERY CARD (BELOW BATTERY) ── */}
                            {(() => {
                                const discountInfo = getDiscountTypeInfo();
                                return (
                                    <div className="mt-3 pt-3 border-t border-white/10 w-full flex flex-col items-center text-center relative z-10">
                                        <span className="text-xs font-black text-white leading-tight">
                                            {discountInfo.title}
                                        </span>
                                        <span className="text-[11px] font-bold text-white/60 mt-0.5 max-w-[260px] leading-snug">
                    ...
                    {/* ── SEPARATE GIFTX WIDGET BLOCK (BELOW WALLET BLOCK, LEFT-ALIGNED — SHOWN ONLY FOR DEPOSIT HOLDERS) ── */}
                    {isDepositActive && (() => {
                        ...
                        >
                            ...
                            <div className="space-y-2.5">
                                {[
                                    ...
                                    { label: 'Текущая скидка', value: `${displayDiscount}% (Базовая: ${discount}%)`, color: 'text-[#00FF41]' },

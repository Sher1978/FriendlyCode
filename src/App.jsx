...
const GlobalBatteryLoader = ({ onComplete }) => {
    ...
    return (
        ...
        >
            {/* Initial full-black cover with top-right REVOO logo fading out in 1.2s */}
            <motion.div
                initial={{ opacity: 1 }}
                animate={{ opacity: 0 }}
                transition={{ duration: 1.2, ease: "easeOut" }}
                className="fixed inset-0 bg-black z-[100] pointer-events-none flex items-start justify-end p-5"
            >
                <img 
                    src="/revoo-logo.png" 
                    alt="REVOO Logo" 
                    className="h-7 sm:h-8 object-contain mix-blend-screen opacity-90 drop-shadow-[0_0_10px_rgba(212,175,55,0.3)] mt-1 mr-2" 
                />
            </motion.div>

            {/* Content manifesting out of blackness over 1.2s */}
            <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                transition={{ duration: 1.2, ease: "easeOut" }}
                className="flex flex-col items-center justify-start h-full w-full relative"
            >
                ...
                {/* Universal header bar spacer matching TestQRPage layout */}
                <div 
                ...
                <div className="flex flex-col items-center justify-start mt-1 px-6 pb-[140px] w-full max-w-md mx-auto z-10 gap-2.5" style={{ msOverflowStyle: 'none', scrollbarWidth: 'none' }}>
                    ...
                    {/* Container with exact p-4 padding matching TestQRPage card layout */}
                    <div className="flex flex-col items-center w-full p-4 relative flex-shrink-0">
                        ...
                        {/* "ВЫЧИСЛЯЕМ ВАШУ СКИДКУ..." placed ABOVE battery, matching discount % height */}
                        ...
                        {/* Battery at exact vertical position */}
